...
export class OperationComponent implements OnInit {
  ...
  op(ship: any) {
    ...
    this.successList;
    this.modalService
      .open(ship, { size: 'xl', ariaLabelledBy: 'modal-basic-title' })
      .result.then(
        (result) => {
          this.closeResult = `Closed with: ${result}`;
        },
  ...
  uploadfile(ship: any) {
    ...
    for (let i = 0; i < x.length; i++) {
      if (
        x[i]['Container No'] == null ||
        x[i]['Port Name'] == null ||
        x[i]['Shipment Data'] == null
      ) {
        // console.log(x[i]);
        this.error.push({
    ...
    this.successEnc = CryptoJS.AES.encrypt(
      JSON.stringify(this.success),
      ''
    ).toString();
    this.errorEnc = CryptoJS.AES.encrypt(
    ...
    var b = CryptoJS.AES.decrypt(this.successEnc, '').toString(
      CryptoJS.enc.Utf8
    );
    ...
    this.errorList = JSON.parse(
      CryptoJS.AES.decrypt(this.errorEnc, '').toString(CryptoJS.enc.Utf8)
    );
    ...
    this.successList = JSON.parse(
      CryptoJS.AES.decrypt(this.successEnc, '').toString(CryptoJS.enc.Utf8)
    );
